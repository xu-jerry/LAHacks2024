...
class Foods(rx.State):
    cur_foods: Dict[str, Dict[str, bool]] = {
        key: {name: False for name in value} for key, value in foods.items()
    }
...
def make_category(food_group: List):
    return rx.box(
        rx.hstack(
            rx.hstack(
                rx.text(food_group[0], margin_right="10px",),
                rx.text(Foods.num_foods[food_group[0].to_string()[1:-1]], color="gray"),
                rx.cond(
            ...
            rx.foreach(
                ...
                lambda food: rx.cond(
                    ...
                    rx.button(
                        ...
                        on_click=lambda: Foods.update_local_inventory(
                            food_group[0], food[0], food[1]
                        ),
                    ),
                    rx.button(
                        food[0],
...
def inventory() -> rx.Component:
    return rx.box(
        rx.hstack(
            rx.text(
                "Inventory", font_size="3rem", font_weight="600"
            ),
        ...
        rx.hstack(
            rx.foreach(
                ...
                lambda appliance: rx.image(
                    src="/inventory/" + appliance + ".svg", width="15em"
                ),
            ),
            display="flex",
            justify_content="space-between",
